...
TSTree *parse_and_update(const char *old_source,
...
int main()
{
	...
	TSTree *tree =
	    ts_parser_parse_string(parser, NULL, original_code, original_length);
	if (!tree) {
	...
	if (result) {
		for (int i = 0; i < result->count; i++) {
			...
			printf("Segment %d: Type %s\t| A[%d:%d], B[%d:%d]\n", i,
			       segment.type == EQUAL    ? "EQUAL"
			       : segment.type == INSERT ? "INSERT"
			       : segment.type == DELETE ? "DELETE"
			                                : "REPLACE",
			       segment.start_a, segment.end_a, segment.start_b,
			       segment.end_b);
		...
		print_side_by_side_diff(original_code, modified_code, result->segments,
		                        result->count);
		printf("\n");
	...
	TSTree *new_tree =
	    ts_parser_parse_string(parser, tree, modified_code, modified_length);
	if (!new_tree) {
