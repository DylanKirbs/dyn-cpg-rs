...
def _add_nodes(
    ...
    for range in changed_ranges:
        if range.start_byte <= node.start_byte and node.end_byte <= range.end_byte:
            props.is_changed = True
        elif node.start_byte <= range.start_byte and range.end_byte <= node.end_byte:
